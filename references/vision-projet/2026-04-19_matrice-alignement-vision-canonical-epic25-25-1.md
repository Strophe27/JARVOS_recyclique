...
**Convention de notation (2026-04-21)** — éviter la confusion **story BMAD** vs **ADR** :

| Écriture | Signification |
|----------|-----------------|
| **25.2**, **25.3** (point) | Stories Epic **25** dans `epics.md` / `sprint-status.yaml` (ex. fermeture ADR *documentaire*). |
| **ADR 25-2**, **ADR 25-3** ou **25-2** / **25-3** dans un contexte ADR | Fichiers d’architecture **acceptés** ; ne pas confondre avec la clé YAML `25-2-fermer-adr-…` qui désigne la **story** porteuse. |

**Hiérarchie en cas de divergence** : pour le pilotage des clés `25-*`, statuts d’**`epic-25`** et liste canonique des stories **25.6–25.15**, **`_bmad-output/implementation-artifacts/sprint-status.yaml`** et **`_bmad-output/planning-artifacts/epics.md`** §25 **priment** sur cette matrice datée du **2026-04-19** ; la matrice reste la trace vision → canon à l’instant T de la story **25.1**.
...
**Référentiels pilotage post–phase doc (2026-04-21)** — pour recoupement sans ouvrir une troisième vérité orpheline :

- `_bmad-output/planning-artifacts/epics.md` (section **Epic 25**, stories **25.6–25.15**)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` (clés `25-6` … `25-15`, `epic-25`)
...
| **PIN kiosque PWA** (secret de poste, lockout métier, offline) | cible | **ADR 25-2 acceptée (2026-04-19).** Cible produit dans la PRD vision ; le **modèle d’architecture** est tranché — le **code** reste soumis au **gel** hors `25-*`, au **NOT READY** PWA massif (readiness) et aux stories explicites (`epics.md`). | `references/vision-projet/2026-04-19_prd-recyclique-architecture-permissions-multisite-kiosques-bmad.md` ; `_bmad-output/planning-artifacts/prd.md` ; `_bmad-output/planning-artifacts/research/technical-alignement-brownfield-prd-recyclique-multisite-permissions-research-2026-04-19.md` ; `references/artefacts/2026-04-19_03_qa2-findings-revisions-cloture-bmad-passe.md` ; `_bmad-output/planning-artifacts/architecture/2026-04-19-adr-pin-kiosque-vs-pin-operateur-secret-poste-step-up-lockout-offline.md` | noyau Epic 25 | **Oui** pour le code **sous** gates gel/readiness/stories — plus « **Non** avant **25.2** » au sens **décision ADR** (25.2 **done**). | Gates process + PWA + backlog kiosque explicite. |
| **Async écritures Paheko** (outbox SQL, idempotence, reprise) | canonique | Chemin nominal actuel ; toute évolution qui change la sémantique → **nouvelle ADR** (l’**ADR 25-3** est **acceptée** ; la **story 25.3** est **done**). | `_bmad-output/planning-artifacts/prd.md` ; `_bmad-output/planning-artifacts/research/technical-alignement-brownfield-prd-recyclique-multisite-permissions-research-2026-04-19.md` ; `_bmad-output/planning-artifacts/implementation-readiness-report-2026-04-19.md` ; `references/artefacts/2026-04-19_03_qa2-findings-revisions-cloture-bmad-passe.md` ; `_bmad-output/planning-artifacts/architecture/2026-04-20-adr-async-paheko-outbox-durable-redis-auxiliaire-ou-trajectoire-hybride.md` | noyau Epic 25 | Oui — déjà livré côté epics 8 / 22–23 ; évolutions sensibles → nouvelle ADR | Ne pas imposer « Redis seul » comme vérité durable (cf. **AR12** + ADR acceptée). |
| **Formulation « file Redis » pour Paheko** (vision) | canonique | **ADR 25-3 acceptée (2026-04-20).** La vision nomme Redis comme file ; le **canon** durable reste l’outbox PostgreSQL ; Redis = **auxiliaire** / transport — voir ADR. | `references/vision-projet/2026-04-19_prd-recyclique-architecture-permissions-multisite-kiosques-bmad.md` ; `_bmad-output/planning-artifacts/prd.md` ; `_bmad-output/planning-artifacts/research/technical-alignement-brownfield-prd-recyclique-multisite-permissions-research-2026-04-19.md` ; `references/artefacts/2026-04-19_03_qa2-findings-revisions-cloture-bmad-passe.md` ; `_bmad-output/planning-artifacts/architecture/2026-04-20-adr-async-paheko-outbox-durable-redis-auxiliaire-ou-trajectoire-hybride.md` | noyau Epic 25 | **Oui** pour impl **conforme** ADR — plus « **Non** avant **25.3** » au sens **décision ADR** (25.3 **done**). | Ne pas réintroduire Redis comme vérité durable concurrente. |
...
| **Device / token matériel kiosque** (IndexedDB, révocation IP) | hors gate | Cible vision Epic 1. **Mise à jour 2026-04-21** : l’**ADR PIN (25-2)** est **acceptée** — ne plus formuler un blocage « tant que l’ADR PIN n’est pas fermée ». Freins réels : **NOT READY** programme PWA (readiness 2026-04-19), **gel** process hors `25-*` tant que **25.6** n’est pas livrée, stories kiosque **explicites** et spec **25.4** §5 (livraison gated). | `references/vision-projet/2026-04-19_prd-recyclique-architecture-permissions-multisite-kiosques-bmad.md` ; `_bmad-output/planning-artifacts/research/technical-alignement-brownfield-prd-recyclique-multisite-permissions-research-2026-04-19.md` ; `_bmad-output/planning-artifacts/implementation-readiness-report-2026-04-19.md` ; `references/artefacts/2026-04-19_03_qa2-findings-revisions-cloture-bmad-passe.md` ; `_bmad-output/planning-artifacts/architecture/2026-04-19-adr-pin-kiosque-vs-pin-operateur-secret-poste-step-up-lockout-offline.md` | future story | **Non** tant que **NOT READY** PWA massif / **gel** / absence de story de dev **hors gate** ne sont pas levés ou cadrés | Hors séquence exécutable immédiate ; spike **25.15** borne le sujet sans livrer PWA. |
...
|------|-----------|----------|
| **Reste documentaire** jusqu’à ADR / spec | Sujet avec statut **ADR requise** ou **hors gate** | **ADR 25-2 et 25-3 acceptés** (plus de blocage « avant 25.2 / 25.3 » au sens décision ADR) ; PWA massif / offline queue restent **hors gate** readiness tant que non levés ; **gel process** distinct (correct course). |
