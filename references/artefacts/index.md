...
| `2026-04-02_03_spec-multi-contextes-invariants-autorisation-v2.md` | Epic 1.3 : spec canonique v2 — entites minimales (site, caisse, session, poste reception, role, groupe, permissions, PIN), invariants zero fuite et bascules de contexte, permissions additives / cles stables / libelles / multi-groupes / autorite backend, step-up (confirmation, PIN, revalidation), blocage / degradation / recalcul ContextEnvelope ; alignement AR39 / AR19 ; ecarts stub `peintre-nano` ; pas d'OpenAPI (Story 1.4). |
| `2026-04-02_04_gouvernance-contractuelle-openapi-creos-contextenvelope.md` | Epic 1.4 : gouvernance normative — hiérarchie AR39 (OpenAPI > ContextEnvelope > NavigationManifest > PageManifest > UserRuntimePrefs), propriétaires et emplacements canoniques, frontières OpenAPI vs CREOS, versionnement et drift, artefacts générés dérivés, flux enums et clés permission (alignement 1.3), `recyclique-api.yaml` et `operationId` stables, extension `data_contract` dans `widget-declaration.schema.json`, runtime Peintre_nano borné. |
| `2026-04-02_05_contrat-minimal-sync-reconciliation-paheko.md` | Epic 1.5 : contrat minimal documentaire sync / réconciliation Recyclique ↔ Paheko — cycle de vie (local, retry, états FR24, résolution, rejet, état final), outbox durable AR11, idempotence AR24, corrélation AR17/AR21 alignée 1.4, FR23/FR25 et blocage sélectif, audit minimal levée quarantaine, FR39 version minimal contract, cohérence OpenAPI AR39 ; hypothèses et gaps Paheko renvoyés Story 1.6. |
| `2026-04-02_06_matrice-integration-paheko-gaps-api.md` | Epic 1.6 : matrice opération → API officielle / plugin minimal / SQL analyse-admin / hors scope v2 ; preuves (liste endpoints, audits migration, brownfield) ; gaps API et inconnues avec conséquence backlog (Epics 6–10, 8) ; garde-fous FR5/FR40/AR9 (pas de SQL transactionnel nominal). |
| `2026-04-02_07_signaux-exploitation-bandeau-live-premiers-slices.md` | Epic 1.7 : signaux d'exploitation pour **bandeau live** et premiers slices — cartographie F1–F6 (horaires effectifs, décalages, exceptions, sync réf. 1.5, contexte 1.3, effectivité caisse/session UX-DR15), matrice cas limites, règles absent/null/explicite, alignement OpenAPI brouillon (`recyclique_exploitation_getLiveSnapshot`) ; pont Epic 4 (4.1–4.4) et report impl. Epic 2.7 — sans widget Peintre. |
