...
| **Mouvement interne** (`cash.transfer`) | **Implémenté (Story 24.8)** | Permission `cash.transfer` (migration `s24_8_cash_internal_transfer_story248.py`), `POST /v1/cash-sessions/{id}/internal-transfers`, journal `PaymentTransaction.nature=cash_internal_transfer`, snapshot `cash_internal_transfer_lines`, builder Paheko rôle `internal_cash_transfer`, UI `/caisse/mouvement-interne`, tests `test_story_24_8_cash_internal_transfer.py` + hub Peintre. |
...
| `cash.transfer` | **Présent** (migration 24.8) | N2–N3 (step-up `cash_session.internal_transfer_step_up` si montant ≥ 500 € ou types sensibles) | Story 24.8 livrée. |
