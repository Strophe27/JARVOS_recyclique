...
Tous les endpoints **métier** habituels sont préfixés par **`/v1`** (ex. `GET /v1/health`). En production, l'API est souvent exposée sous `/api` (ex. `GET /api/v1/health`).

**Réconciliation (2026-04-02) :** cette liste a été **alignée** sur un export **`/v1/openapi.json`** de l'app tournant dans la stack Docker **`recyclic-local`** (`recyclique-1.4.4/docker-compose.yml`, port API typique **8000**, Swagger `http://localhost:8000/docs`). Détail des écarts historiques et méthode : `references/artefacts/2026-04-02_02_audit-brownfield-backend-api-donnees-critiques.md` §1 bis.

**Maintenance (équipe solo — règle simple) :** dès que tu **modifies des routes FastAPI** sous `/v1` (ou la racine `/`, `/health`), ou juste **avant** de figer un contrat OpenAPI / une release qui engage le front : (1) démarre l’API, (2) exporte `http://localhost:8000/v1/openapi.json`, (3) compare méthode+chemin avec ce fichier et mets les tableaux à jour. Optionnel : copie datée du JSON sous `references/artefacts/` comme preuve (voir `2026-04-02_08_openapi-recyclique-live-recyclic-local.json`). Tu n’as **pas** besoin d’un autre rituel ni de te souvenir d’un seuil « grosse évolution » : **changement de routes = mise à jour de cette liste**.

---

## Racine (hors préfixe `/v1`)

| Méthode | Chemin | Utilité |
|--------|--------|---------|
| GET | `/` | Point d'entrée racine (selon déploiement / proxy). |
| GET | `/health` | Santé simple hors montage `/v1` (complément de `GET /v1/health`). |
...
**Historique :** `POST /v1/users/link-telegram` figurait dans d'anciennes versions ; **non présent** dans l'export OpenAPI live réconcilié (2026-04-02). Vérifier une branche / build avant de l'inclure dans un contrat v2.
...
**Historique (hors OpenAPI live réconcilié) :** `POST /v1/deposits/from-bot` et `POST /v1/deposits/{deposit_id}/classify` — **retirés** du surface HTTP dans la consolidation (voir `references/artefacts/2026-03-26_04_paquet-3-suppression-routes-api-bot-historique.md` et paquets associés).
...
*Note (OpenAPI live 2026-04-02) :* pas de `GET /v1/admin/users/{user_id}` seul dans l'export — utiliser **GET** `/v1/admin/users`, **GET** `.../history`, **PUT** `.../{user_id}` pour mise à jour.
...
## Monitoring (classification, cache, emails, auth, sessions) (`/v1/monitoring`)
...
| GET | `/v1/monitoring/auth/metrics` | Métriques d'authentification |
| GET | `/v1/monitoring/auth/metrics/prometheus` | Métriques auth au format Prometheus |
| POST | `/v1/monitoring/auth/metrics/reset` | Réinitialiser les métriques auth |
| GET | `/v1/monitoring/sessions/metrics` | Métriques de sessions |
| GET | `/v1/monitoring/sessions/metrics/prometheus` | Métriques sessions au format Prometheus |
| POST | `/v1/monitoring/sessions/metrics/reset` | Réinitialiser les métriques sessions |
...
## Test auth (développement) — hors schéma live réconcilié

**Statut :** `GET /v1/test-auth/test` **n'apparaît pas** dans l'export OpenAPI live (stack `recyclic-local`, 2026-04-02). Conserver cette section comme **historique** uniquement ; ne pas figer dans un contrat v2 sans vérification sur la branche déployée.

| Méthode | Chemin | Utilité |
|--------|--------|---------|
| GET | `/v1/test-auth/test` | Endpoint de test d'authentification (si activé dans le build) |

---

*Document aligné sur le code sous `recyclique-1.4.4/` et réconciliation OpenAPI (voir entête). Détail des paramètres et schémas : en dev, **`http://localhost:8000/docs`** et **`http://localhost:8000/v1/openapi.json`** (ports selon compose).*
