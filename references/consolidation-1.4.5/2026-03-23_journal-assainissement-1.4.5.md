...
## Micro-lot FE-TG-01 — Suppression `frontend/src/types/simple.ts` (orphelin)

### Theme
Verifier et retirer le fichier de types `simple.ts`, reliquat documente comme contournement d'imports ; aucun consommateur dans la base `recyclique-1.4.4/frontend`.

### Verification usages
- Worktree de depart : **propre** (`chore/v1.4.5-consolidation`, aligne sur `origin`, aucun diff avant action).
- Recherche dans `recyclique-1.4.4/frontend` : aucun import `types/simple`, `@/types/simple`, ni chemin equivalent ; les seules occurrences du mot « simple » sont des commentaires ou du texte UI, pas ce module.
- Mention residuelle hors code : `recyclique-1.4.4/docs/pending-tech-debt/story-tech-debt-remove-cashier-role.md` (liste de chemins) ; **non modifiee** dans ce lot (perimetre journal + suppression fichier uniquement).

### Fichiers touches
- `recyclique-1.4.4/frontend/src/types/simple.ts` (supprime)
- `references/consolidation-1.4.5/2026-03-23_journal-assainissement-1.4.5.md`

### Validation
- `npm run build` (frontend) : **OK** — bundle produit ; avertissement esbuild preexistant sur `LegacyImport.tsx` (attribut JSX `searchable` duplique), sans lien avec ce lot.
- `npm run lint` : **echec global preexistant** (centaines d'erreurs / warnings dans le projet) ; **aucune regression** liee a `simple.ts` (fichier absent des rapports).

### Resultat / mini-QA
- Fichier **100 % orphelin** au sens imports / references TypeScript ; suppression sans impact compile.
- Lot ferme ; **pret commit/push** si le worktree reste limite a la suppression + ce journal.

---

## Micro-lot FE-TG-02 — `authService.ts` : types + login alignes sur `POST /v1/auth/login`

### Theme
Retirer la copie locale divergente de `LoginRequest` / `AuthUser` / `LoginResponse` (ancien corps type `telegram_id` et `POST /auth/login`) ; aligner sur le flux reel (store + OpenAPI) sans modifier `generated/*`.

### Verification / constat
- Worktree de depart : **propre** (`chore/v1.4.5-consolidation`, aucun diff avant modifications).
- Consommateurs : seuls `dashboardService` et `reportsService` importent `getAuthHeader` depuis ce module ; aucun appelant a `login()` dans `recyclique-1.4.4/frontend/src`.
- Flux reel : `authStore.login` → `AuthApi.apiv1authloginpost` → `POST /v1/auth/login`, corps `LoginRequest` `{ username, password }` (`generated/types`).
...
### Validation
- `npx vitest run src/services/__tests__/dashboardService.test.ts` : **10 tests OK**
- `npx eslint src/services/authService.ts` : **0 probleme**
- Diagnostics IDE sur `authService.ts` : **0 probleme**

### Resultat / mini-QA
- Types reexportes depuis `../generated/types` (contrat unique avec l’API generee) ; `login()` utilise `/v1/auth/login` comme `AuthApi`.
- Lot ferme ; **pret commit/push** si le worktree reste limite a `authService.ts` + ce journal.

---

## Micro-lot FE-TG-04 — `useAuth.ts` : `telegram_id` aligne sur `authStore`

### Theme
Reduire la dualite de contrat entre le hook legacy `hooks/useAuth.ts` (localStorage `user`) et `stores/authStore.ts` sur le seul champ `telegram_id`, sans fusion des deux chemins d'auth ni refactor large.

### Verification worktree
- Branche `chore/v1.4.5-consolidation` ; worktree **propre** avant modification.

### Fichiers touches
- `recyclique-1.4.4/frontend/src/hooks/useAuth.ts` : `User.telegram_id` en `string | number` + JSDoc aligne sur le store ; `refreshAuth` : `catch` sans binding (paramètre `error` inutilise, conforme ESLint sur le fichier touche).
- `recyclique-1.4.4/frontend/src/test/hooks/useAuth.test.ts` : test de non-regression chargement `localStorage` (nombre + chaîne non numerique).
- `references/consolidation-1.4.5/2026-03-23_journal-assainissement-1.4.5.md`

### Validation
- `npx vitest run src/test/hooks/useAuth.test.ts` : **OK** (15 tests)
- `npx eslint src/hooks/useAuth.ts src/test/hooks/useAuth.test.ts` : **OK** (0 probleme)

### Resultat / mini-QA
- Le typage et la doc du hook ne sous-typent plus `telegram_id` en `string` seul : cohérent avec `authStore.User` et les payloads API (historique nombre, tests chaîne).
- Aucun import croise hook ↔ store (perimetre minimal).
- Lot ferme ; **pret commit/push** si le diff reste limite aux fichiers ci-dessus.
...
## Micro-lot FE-TG-07 — `UserDetailView` : titre et avatar alignes sur `fullNameOrUsernameOrTelegramFallback`

### Theme
Harmoniser l'en-tete detail utilisateur admin avec le helper deja utilise par `convertToAdminUser` / liste : `user.full_name` puis repli `fullNameOrUsernameOrTelegramFallback` (inclut `User {telegram_id}` lorsque seuls prenom / nom / username absents). Initiale avatar derivee du libelle affiche. Retrait de helpers `getStatusColor` / `getStatusLabel` non utilises (lint). Ajustement du test « informations selectionnees » : attendre « Bénévole » (role USER) au lieu de libelles obsoletes.
...
### Fichiers touches
- `recyclique-1.4.4/frontend/src/components/business/UserDetailView.tsx`
- `recyclique-1.4.4/frontend/src/components/business/__tests__/UserDetailView.test.tsx`
- `references/consolidation-1.4.5/2026-03-23_journal-assainissement-1.4.5.md`

### Validation
- `npx vitest run src/components/business/__tests__/UserDetailView.test.tsx` : **7 tests OK**
- `npx eslint` sur les deux fichiers touches : **OK**
- `generated/*` : **non modifie**

### Resultat / mini-QA
- Cas `full_name` vide + seul `telegram_id` : titre `User …` coherent avec la ligne `@` et avec `userDisplay.ts` ; pas de divergence avec la table utilisateurs.
- Lot ferme ; **pret commit/push** si le diff reste limite aux fichiers ci-dessus + ce journal.
...
## Micro-lot FE-TG-09 — Mot de passe oublié : validation client Zod (email)

### Theme
Schéma `forgotPasswordFormSchema` (`src/schemas/forgotPasswordForm.ts`) : email trim, requis, format valide. Page `ForgotPassword.tsx` : `noValidate`, erreur client prioritaire affichée avec les erreurs store dans le même bloc `#email-error`, trim via données parsées avant `forgotPassword()`.

### Verification worktree
- Branche `chore/v1.4.5-consolidation` ; worktree **propre** avant modification (après push `FE-TG-03`).

### Fichiers touches
- `recyclique-1.4.4/frontend/src/schemas/forgotPasswordForm.ts` (nouveau)
- `recyclique-1.4.4/frontend/src/pages/ForgotPassword.tsx`
- `recyclique-1.4.4/frontend/src/test/pages/ForgotPassword.test.tsx`
- `references/consolidation-1.4.5/2026-03-23_journal-assainissement-1.4.5.md`

### Validation
- `npx vitest run src/test/pages/ForgotPassword.test.tsx` : **13 tests OK**
- `npx eslint` sur les fichiers touches : **0 erreur**
- `generated/*` : **non modifie**

### Resultat / mini-QA
- Soumission email vide ou invalide : pas d'appel `forgotPassword` ; messages Zod cohérents avec les tests mis à jour (fin de la dépendance à `required` + validité HTML5 pour le cas invalide).
- Lot ferme ; **pret commit/push** si le diff reste limite aux fichiers ci-dessus + ce journal.
