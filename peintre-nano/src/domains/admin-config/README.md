...
**Groupes, catégories, journal d'audit** : **`/admin/groups`**, **`/admin/categories`**, **`/admin/audit-log`** — `page_key` **`transverse-admin-*`**, widgets **`admin.groups.demo`**, **`admin.categories.demo`**, **`admin.audit-log.demo`** (`AdminGroupsWidget`, **`AdminCategoriesWidget`**, **`AdminAuditLogWidget`**) ; résolution **`RuntimeDemoApp`** + manifestes dans **`runtime-demo-manifest.ts`**. Ces chemins ont aussi des entrées **`transverse-admin-*`** dans **`navigation-transverse-served.json`** (copie **`public/manifests/navigation.json`**) pour la barre d'outils transverse, en complément du hub **`/admin`** (**`admin.legacy.dashboard.home`**).
