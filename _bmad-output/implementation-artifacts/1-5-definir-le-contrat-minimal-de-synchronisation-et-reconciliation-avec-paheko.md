...
- [x] Croiser **`references/paheko/`** (API, brownfield) et **`references/migration-paheko/`** pour **ne pas** inventer des capacités Paheko non étayées ; signaler **hypothèses** et **gaps** renvoyés à la **Story 1.6** (matrice / gaps API).
...
- **Références** : `epics.md` Story 1.5 ; **FR39**, **FR23–FR25** ; `references/paheko/`, `references/migration-paheko/` ; artefacts **1.4** / `contracts/README.md` pour alignement erreurs (`code`, `detail`, `retryable`, `state`, `correlation_id`, **AR21**).
...
- [Source : `references/migration-paheko/` — index et audits brownfield si besoin de cadrage réaliste]
