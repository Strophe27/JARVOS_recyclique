...
- **Docs existantes** : `README.md` (Docker dev) ; runbook PG spike ; stories **10.6b–10.6e** ; manifeste **`doc/supported-stack-official.yaml`** et guide **`doc/installation-stack-officielle.md`** livrés par **10.6** (snapshot brownfield ci-dessus = état pré-DS).
...
- Tests : 12 passed (smokes 10.6) + 7 passed / 2 skipped (10.6c/e) ; `docker compose config` non exécuté (Docker absent sur agent cloud).
