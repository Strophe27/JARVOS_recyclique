...
**Lecture plancher Option C (post-D1) :** le tableau ci-dessus décrit la **cible G-vendable**. Pour le **plancher L0** déjà livré ou en cours sans élargir **L1** : socle caisse/réception/sync (**epics 6–8**), **config admin simple** (**9.6**), liaison clôture (**9.10**), gates ship **Epic 10** ; les **modules métier D** (éco-org, adhérents, etc.) = **L1** — **un à la fois**, **après** tranche **10.1–10.3** et décision **D2/D7**. **HelloAsso** : **exclu de L1** tant que **D3** parking (ligne tableau ci-dessus **hors L0/L1** ; stories **9.4/9.5** = cadrage doc uniquement).
