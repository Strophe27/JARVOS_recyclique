...
2. **Couche métier cible kiosque** : les seuils **vision** (**3** échecs → **soft-lock** **30 s**, **5** → **escalade admin / superviseur**) sont **retenus comme objectif produit par défaut** pour le **PIN kiosque** — **pas** comme constantes figées dans le code pour toutes les structures.
3. **Paramétrage organisationnel (SuperAdmin)** : la politique de lockout **kiosque** (nombre d’essais avant soft-lock, durée de soft-lock, seuil avant escalade, éventuels **profils** ou **schémas** prédéfinis) doit être **hautement paramétrable** côté **SuperAdmin** (activation / désactivation de jeux de règles, ajustement des durées et compteurs dans des **bornes** à fixer par stories d’implémentation pour éviter des réglages absurdes ou dangereux). Les valeurs **3 / 30 s / 5** restent la **référence par défaut** (onboarding, doc, environnements sans config spécifique) ; une organisation peut adopter un autre schéma ou désactiver certaines briques **si** le produit le permet et que la traçabilité / l’audit restent assurés. L’implémentation devra prévoir **compteurs par identité / poste** (et non seulement par IP), **persistance locale** pendant offline et **synchronisation** ou **fusion de règles** au retour en ligne — détail réservé aux stories aval (**sans** supprimer le rate limit opérateur ni réduire §11.2).
...
- **Paramétrage** : surface **SuperAdmin** + modèle de données pour profils de lockout, migrations, tests de non-régression quand une org change de schéma — charge UX et technique supplémentaire.
...
## Stories aval directement concernées (ADR **accepted** ; autres freins : gel process, readiness PWA, spec 25.4)

Les identifiants ci‑dessous sont ceux de **`_bmad-output/planning-artifacts/epics.md`**. L’ADR est **norme approuvée** ; une story peut rester **non ready-for-dev** pour des raisons **autres** que le statut ADR (gel hors `25-*`, verdict **NOT READY** PWA massif, gate API P0, dépendances code).
...
- **Story 25.4** — **Done** ; la spec convergée (`2026-04-20-spec-socle-multisite-permissions-invariants-poste-kiosque-projection-recyclique-paheko.md`) **consomme** cette ADR comme norme pour poste/kiosque et bascule de contexte.

## Suivi BMAD

- **Acceptation (2026-04-19)** : statut **`accepted`** après décision explicite du décideur produit (séparation deux PIN, cadre hybride borné, seuils vision comme défaut paramétrable SuperAdmin — voir §4).
- Les implémentations futures citent cet ADR comme **norme approuvée** ; toute évolution substantielle repasse par **correct course** ou nouvelle ADR.
- **Story 25.3** (async Paheko), **25.4** (socle multisite / poste), **25.5** (readiness) : **done** au pilotage documentaire ; le **gel** process hors `25-*` reste régi par le **correct course** jusqu’à levée documentée (indépendamment de la clôture des stories **25.1–25.5**).

> **Cohérence rédactionnelle post–25.6 (sans rouvrir la décision)** — La **levée** du gel **process** pour l’**exécution** BMAD **hors** clés **`25-*`** est tracée sous **`GEL_DOC`** : `_bmad-output/planning-artifacts/2026-04-20-addendum-levee-gel-process-correct-course-story-25-6.md` + `_bmad-output/implementation-artifacts/sprint-status.yaml`. Cette ADR reste la **norme** pour PIN / poste / kiosque ; le **`GEL_DOC`** ne modifie **aucun** choix d’architecture ici.
