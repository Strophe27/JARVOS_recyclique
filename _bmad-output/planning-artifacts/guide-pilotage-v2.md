...
- **Epic 1** — Prérequis structurants, modèle multi-contextes, gouvernance OpenAPI/CREOS (jalon FR73 / schémas minimaux selon epics). *Statut epic : `done` dans `sprint-status.yaml`.*
- **Epic 2** — Socle backend brownfield v2 (auth, contexte, permissions, persistance, signaux bandeau). *Idem.*
- **Epic 3** — Socle Peintre_nano (shell, slots, validation CREOS, mocks permis avant C1). *Idem.*
- **Epic 4** — Preuve chaîne modulaire **bandeau live**.
- **Epic 5** — Shell, dashboard, admin transverses dans Peintre.
- **Epic 6** — Caisse v2 exploitable. *Epic clos dans `sprint-status.yaml` ; le critère brownfield qui a structuré le pilotage reste rappelé en §7 (gate + registre `2026-04-08_04_caisse-v2-exploitabilite-terrain-epic6.md`).*
- **Epic 7** — Réception v2 exploitable. *Statut epic : `done` dans `sprint-status.yaml`.*
- **Epic 8** — Articulation comptable réelle Paheko. *Idem.*
