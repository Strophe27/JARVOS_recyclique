# Script pour déplacer les stories restantes qui ont Gate: PASS dans leur contenu

$filesWithGatePass = Get-ChildItem "docs/stories/story-*.md" | Where-Object {
    (Get-Content $_.FullName -Raw) -match 'Gate:\s*\*?PASS\*?'
}

Write-Host "Stories restantes avec Gate PASS trouvées: $($filesWithGatePass.Count)"
...
Write-Host "Migration des stories Gate PASS restantes terminée: $($filesWithGatePass.Count) fichiers déplacés"
