...
> Note 2026-03-26 : document conserve comme archive de design de tests.
> Les cas critiques qui appellent `POST /deposits/{id}/classify` decrivent un flux bot historique.
> En deploiement courant, cette route n'est plus un parcours actif de reference et est desactivee (`410 Gone`).
> Ne pas reutiliser ces scenarios comme oracle d'execution sans verifier le contrat runtime actuel.
