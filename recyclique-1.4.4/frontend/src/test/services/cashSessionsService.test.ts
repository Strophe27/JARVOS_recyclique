/**
 * Tests unitaires pour cashSessionsService.exportBulk() (Story B45-P1)
 * 
 * Valide :
...
import { cashSessionsService, CashSessionFilters } from '../../services/cashSessionsService'
...
describe('cashSessionsService.exportBulk', () => {
  const mockFilters: CashSessionFilters = {
  ...
  it('should call API with correct parameters for CSV export', async () => {
    ...
    const mockResponse = {
      data: mockBlob,
      headers: {
        'content-disposition': 'attachment; filename="export_sessions_caisse_20250127_120000.csv"'
      }
    }

    vi.mocked(axiosClient.post).mockResolvedValue(mockResponse as any)

    await cashSessionsService.exportBulk(mockFilters, 'csv')
    ...
    expect(axiosClient.post).toHaveBeenCalledWith(
      '/v1/admin/reports/cash-sessions/export-bulk',
      {
        filters: {
          ...
          operator_id: mockFilters.operator_id,
          site_id: mockFilters.site_id,
  ...
  it('should call API with correct parameters for Excel export', async () => {
    ...
    const mockResponse = {
      data: mockBlob,
      headers: {
        'content-disposition': 'attachment; filename="export_sessions_caisse_20250127_120000.xlsx"'
      }
    }

    vi.mocked(axiosClient.post).mockResolvedValue(mockResponse as any)

    await cashSessionsService.exportBulk(mockFilters, 'excel')
  ...
  it('should handle errors and throw', async () => {
    ...
    await expect(cashSessionsService.exportBulk(mockFilters, 'csv')).rejects.toThrow('Network error')
  })
  ...
  it('should handle API errors with error message', async () => {
    ...
    await expect(cashSessionsService.exportBulk(mockFilters, 'csv')).rejects.toEqual(mockError)
  })
  ...
  it('should handle empty filters', async () => {
    ...
    await cashSessionsService.exportBulk({}, 'csv')
