...
def test_create_duplicate_name_raises_validation_error():
    ...
    with pytest.raises(ValidationError, match="already exists"):
        service.create_category(CategoryCreate(name="Dup"))
    db.add.assert_not_called()


def test_create_invalid_parent_id_format_raises_validation_error():
    db = MagicMock()
    chain = MagicMock()
    fmock = MagicMock()
    db.query.return_value = chain
...
def test_create_parent_missing_raises_validation_error():
    db = MagicMock()
    chain = MagicMock()
    fmock = MagicMock()
    db.query.return_value = chain
...
def test_create_exceeds_max_depth_raises_validation_error(_mock_depth):
    db = MagicMock()
    chain = MagicMock()
    fmock = MagicMock()
    db.query.return_value = chain
...
def test_create_under_priced_parent_clears_parent_prices(mock_read_cls, _mock_depth):
...
def test_create_integrity_error_raises_validation_error():
    db = MagicMock()
    chain = MagicMock()
    db.query.return_value = chain
    ...
    service = CategoryService(db)
    with pytest.raises(ValidationError, match="already exists"):
        service.create_category(CategoryCreate(name="Unique"))
...
def test_create_category_route_maps_validation_error(admin_client):
    with patch(
        ...
        mock_cls.return_value.create_category = MagicMock(
