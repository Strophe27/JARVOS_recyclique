...
class TestPermissionEndpoints:
    ...
    def test_list_permissions_success(self, client: TestClient, admin_token: str, db_session: Session):
        ...
        # SQLite tests : pas de seed Alembic ; liste peut être vide ou peuplée selon le test.
    ...
    def test_get_permission_success(
        ...
        response = client.get(
            f"/v1/admin/permissions/{sample_permission.id}",
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        data = response.json()
...
class TestGroupEndpoints:
    ...
    def test_get_group_success(
        ...
        response = client.get(
            f"/v1/admin/groups/{sample_group.id}",
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_assign_permissions_to_group(
        ...
        response = client.post(
            f"/v1/admin/groups/{sample_group.id}/permissions",
            json=assign_data,
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
    ...
    def test_remove_permission_from_group(
        ...
        # Then remove it
        response = client.delete(
            f"/v1/admin/groups/{sample_group.id}/permissions/{sample_permission.id}",
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_assign_users_to_group(
        ...
        response = client.post(
            f"/v1/admin/groups/{sample_group.id}/users",
            json=assign_data,
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
    ...
    def test_remove_user_from_group(
        ...
        # Then remove them
        response = client.delete(
            f"/v1/admin/groups/{sample_group.id}/users/{regular_user.id}",
            headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        data = response.json()
