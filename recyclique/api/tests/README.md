...
Le script démarre `postgres` et `redis`, prépare `recyclic_test` si besoin, génère `openapi.json`, puis lance **un sous-ensemble minimal** (auth + infra) via le service `api` avec `TESTING=true` et `ENVIRONMENT=test`. Les schéma et tables sont gérés par `conftest.py` (`create_tables_if_not_exist`), pas par une variable d'environnement dédiée aux migrations.
...
Les options `-v` et `--tb=short` viennent de `api/pyproject.toml` (`[tool.pytest.ini_options].addopts`), pas de la ligne de commande.
...
- **`api/pyproject.toml`** — `[tool.pytest.ini_options]` : `testpaths`, `pythonpath`, `addopts`, marqueurs enregistrés (ex. `no_db`)
