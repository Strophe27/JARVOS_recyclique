...
@pytest.mark.asyncio
async def test_restore_already_active_raises_validation_error():
    db = MagicMock()
    chain = MagicMock()
    ...
    with pytest.raises(ValidationError, match="Category is already active"):
        await service.restore_category(cid)


@pytest.mark.asyncio
async def test_restore_archived_sets_deleted_at_none_and_commits():
    db = MagicMock()
    chain = MagicMock()
    db.query.return_value = chain
    category = MagicMock()
    ...
    with patch.object(CategoryRead, "model_validate", return_value=fake_read) as mv:
        result = await service.restore_category(cid)
...
def test_post_restore_route_maps_validation_error(admin_client):
    ...
    with patch(
        ...
        mock_cls.return_value.restore_category = AsyncMock(
...
def test_post_restore_route_not_found_when_service_returns_none(admin_client):
    with patch(
        ...
        mock_cls.return_value.restore_category = AsyncMock(return_value=None)
        response = admin_client.post(f"{_V1}/categories/{uuid4()}/restore")
