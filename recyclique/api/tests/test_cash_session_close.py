...
def _close_headers(user_id) -> dict:
    return {
        ...
        "X-Step-Up-Pin": _TEST_STEP_UP_PIN,
    }
...
class TestCashSessionClose:
    ...
    def test_close_session_other_operator_forbidden(self, client: TestClient, db_session: Session, test_cash_session: CashSession):
        ...
        response = client.post(
            ...
            headers=_close_headers(other_user.id),
        )

        assert response.status_code == 403
        d = response.json()["detail"]
        msg = d.get("message", d) if isinstance(d, dict) else d
        assert "accès non autorisé" in str(msg).lower()
