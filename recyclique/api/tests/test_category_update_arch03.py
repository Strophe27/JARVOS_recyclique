...
def test_update_duplicate_name_raises_validation_error():
    ...
    with pytest.raises(ValidationError, match="already exists"):
        service.update_category(cid, CategoryUpdate(name="NewName"))
    db.commit.assert_not_called()


def test_update_invalid_parent_id_format_raises_validation_error():
    db = MagicMock()
    chain = MagicMock()
    db.query.return_value = chain
    category = MagicMock()
    ...
    with pytest.raises(ValidationError, match="Invalid parent_id format"):
        service.update_category(str(uuid4()), CategoryUpdate(parent_id="not-a-uuid"))


def test_update_price_when_has_children_raises_conflict_error():
    db = MagicMock()
    chain = MagicMock()
    db.query.return_value = chain
    category = MagicMock()
    ...
    with pytest.raises(ConflictError, match="Cannot set prices on a category that has subcategories"):
        service.update_category(str(uuid4()), CategoryUpdate(price="10"))


@patch("recyclic_api.services.category_service.CategoryRead")
def test_update_under_priced_parent_clears_parent_prices(mock_read_cls):
...
def test_put_category_route_maps_validation_error(admin_client):
    with patch(
        ...
        mock_cls.return_value.update_category = MagicMock(
...
def test_put_category_route_not_found_when_service_returns_none(admin_client):
    with patch(
        ...
        mock_cls.return_value.update_category = MagicMock(return_value=None)
        response = admin_client.put(
