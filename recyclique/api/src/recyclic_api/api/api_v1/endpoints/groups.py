    ...
    try:
        ...
        for group in groups:
            response.append(GroupResponse(
                ...
                site_id=str(group.site_id) if group.site_id else None,
...
def create_group(
    ...
    key = (group_data.key or "").strip() or slugify_label_to_key(group_data.name)
    existing_key = db.execute(select(Group).where(Group.key == key)).scalar_one_or_none()
    if existing_key:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Un groupe avec la clé '{key}' existe déjà",
        )

    site_uuid: Optional[UUID] = None
    if group_data.site_id:
        try:
            site_uuid = UUID(group_data.site_id)
        except ValueError:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
def update_group(
    ...
    if group_data.site_id is not None:
        if group_data.site_id == "":
            group.site_id = None
        else:
            try:
                group.site_id = UUID(group_data.site_id)
            except ValueError:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="site_id invalide",
