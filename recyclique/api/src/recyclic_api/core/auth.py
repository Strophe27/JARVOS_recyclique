...
def user_has_permission(user: User, permission_name: str, db: Session) -> bool:
    ...
    """
    return user_has_effective_permission(user, permission_name, db)
...
def get_user_permissions(user: User, db: Session) -> List[str]:
    ...
    Délègue à ``compute_effective_permission_keys`` (Story 2.3) — même source que
    l'enveloppe de contexte et ``GET /v1/users/me/permissions``.
    ...
    Returns:
        List of permission keys (``Permission.name``) the user has
    """
    return compute_effective_permission_keys(user, db)
