...
class ReceptionLiveStatsService:
    ...
    @staticmethod
    def _site_uuid(site_id: Optional[str]) -> Optional[uuid.UUID]:
        if site_id is None:
            return None
        return uuid.UUID(site_id) if isinstance(site_id, str) else site_id
    ...
    def _calculate_turnover_24h(self, site_id: Optional[str], threshold: datetime, start_of_today: datetime) -> Decimal:
        ...
        if site_uuid is not None:
            query = query.filter(CashSession.site_id == site_uuid)
    ...
    def _calculate_weight_in(self, site_id: Optional[str], threshold: datetime, start_of_today: datetime) -> Decimal:
        ...
        if site_uuid is not None:
            open_weight_query = open_weight_query.join(
                User, PosteReception.opened_by_user_id == User.id
            ).filter(User.site_id == site_uuid)
        open_weight_query = open_weight_query.filter(
            and_(
                TicketDepot.status == TicketDepotStatus.OPENED.value,
                # Exclude deferred tickets: only include tickets from posts opened today
                PosteReception.opened_at >= start_of_today,
        ...
        if site_uuid is not None:
            closed_weight_query = closed_weight_query.join(
                User, PosteReception.opened_by_user_id == User.id
            ).filter(User.site_id == site_uuid)
        closed_weight_query = closed_weight_query.filter(
            and_(
                TicketDepot.status == TicketDepotStatus.CLOSED.value,
                TicketDepot.closed_at.isnot(None),
                TicketDepot.closed_at >= threshold,
    ...
    def _calculate_weight_out(self, site_id: Optional[str], threshold: datetime, start_of_today: datetime) -> Decimal:
        ...
        if site_uuid is not None:
            poids_ventes_query = poids_ventes_query.filter(CashSession.site_id == site_uuid)
        ...
        if site_uuid is not None:
            poids_exit_reception_query = poids_exit_reception_query.join(
                User, PosteReception.opened_by_user_id == User.id
            ).filter(User.site_id == site_uuid)
        poids_exit_reception_query = poids_exit_reception_query.filter(
            and_(
    ...
    def _calculate_cash_stats(
        ...
        if site_uuid is not None:
            sales_query = sales_query.filter(CashSession.site_id == site_uuid)
