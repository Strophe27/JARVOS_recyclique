 ...
 * Uses fixed positional AZERTY mapping for category selection
 *
 * Features:
...
// Position to key mapping for cash interface
...
export class CashKeyboardShortcutHandler {
  ...
  private maxPositions: number = 26;
  ...
  initialize(
    ...
    this.maxPositions = Math.min(maxPositions, 26);
   ...
   * Prevents when focused on input/textarea/select elements
   */
...
export const getPositionsInRow = (row: number): number[] => {
  switch (row) {
    ...
    case 3:
      return Array.from({ length: 6 }, (_, i) => i + 21);
    default:
      return [];
  }
};
